...
import { settingsYamlSchema } from "@/lib/schemas";
...
export async function GET(request: NextRequest) {
  ...
  const content = getConfigYaml(ctx.tenant.slug);
  if (content === null) {
...
export async function PUT(request: NextRequest) {
  ...
  const parsed = settingsYamlSchema.safeParse(raw);
  if (!parsed.success) {
  ...
  const before = getConfigYaml(ctx.tenant.slug);
  const ok = updateConfigYaml(ctx.tenant.slug, parsed.data.content);
  if (!ok) {
    return NextResponse.json({ error: "Failed to update" }, { status: 500 });
  }
  await audit("config.settings.update", "config.yaml", {
